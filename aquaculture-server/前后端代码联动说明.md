...
本文档详细说明水产养殖计划与产量统计系统中前后端代码的联动机制，包括 API 接口对接、数据格式、权限控制、错误处理等方面的实现细节。
...
**后端实现** (`MedicinePurchaseServiceImpl.java`):
...
public boolean savePurchase(MedicinePurchase purchase) {
    ...
    if (inventory == null) {
        ...
        inventory.setMedicineName(purchase.getMedicineName());
        inventory.setMedicineType(purchase.getMedicineType());
        inventory.setStock(purchase.getPurchaseAmount());
        inventoryService.save(inventory);
    } else {
        // 已有药品，累加库存
        inventory.setStock(inventory.getStock().add(purchase.getPurchaseAmount()));
...
**前端实现** (`MedicinePurchase.vue`):

```javascript
const handleSubmit = async () => {
  const res = await savePurchase(medicinePurchaseForm.value)
  if (res.code === 200) {
...
**后端实现** (`MedicineUsageServiceImpl.java`):
...
public boolean saveUsage(MedicineUsage usage) {
    ...
    if (inventory != null) {
        BigDecimal newStock = inventory.getStock().subtract(usage.getDosage());
...
**前端实现** (`MedicineUsage.vue`):

```javascript
const handleSubmit = async () => {
  const res = await saveUsage(usageForm.value)
  if (res.code === 200) {
    ElMessage.success('新增成功')
    // 触发刷新药品库存页面
    window.dispatchEvent(new CustomEvent('refresh-medicine-inventory'))
...
**后端实现** (`AquaculturePlanServiceImpl.java`):

```java
public boolean updatePlan(AquaculturePlan plan) {
    AquaculturePlan oldPlan = this.getById(plan.getPlanId());
    
    // 状态变更验证
    if (oldPlan.getStatus() == 1) { // 已通过
        // 只能修改为执行中(3)或已完成(4)
        if (plan.getStatus() != 1 && plan.getStatus() != 3 && plan.getStatus() != 4) {
            throw new RuntimeException("已通过的计划只能修改为执行中或已完成");
        }
    } else if (oldPlan.getStatus() == 3) { // 执行中
        // 只能修改为已完成(4)或保持执行中(3)
        if (plan.getStatus() != 3 && plan.getStatus() != 4) {
            throw new RuntimeException("执行中的计划只能修改为已完成");
        }
    } else if (oldPlan.getStatus() == 4) { // 已完成
        // 不能再修改状态
        if (plan.getStatus() != 4) {
            throw new RuntimeException("已完成的计划不能再修改状态");
        }
    }
...
const handleSubmit = async () => {
  // 状态变更验证
  if (originalPlanStatus.value !== 1 && (planForm.status === 3 || planForm.status === 4)) {
    ElMessage.error('只有已通过审核的计划才能修改为执行中或已完成')
    return
  }
  
  if (originalPlanStatus.value === 1 && planForm.status !== 1 && planForm.status !== 3 && planForm.status !== 4) {
    ElMessage.error('已通过的计划只能修改为执行中或已完成')
    return
  }
  
  // 提交更新
  const res = await updatePlan(planForm.value)
  // ...
}
```
...
**后端接口** (`AquaculturePlanController.java`):
...
**前端使用** (`FeedUsage.vue`, `MedicineUsage.vue`, `YieldList.vue` 等):
...
**后端接口** (`YieldStatisticsController.java`):
...
**前端实现** (`SalesRecord.vue`):

```javascript
const handlePlanChange = async (planId) => {
  if (planId) {
    // 加载该计划的产量统计
    const res = await getYieldStatisticsByPlanId(planId)
    if (res.code === 200) {
      yieldStatisticsList.value = res.data
      // 如果只有一个，自动选中
      if (res.data.length === 1) {
        salesForm.value.yieldId = res.data[0].yieldId
      }
    }
  } else {
    yieldStatisticsList.value = []
    salesForm.value.yieldId = null
  }
}
```
...
**触发刷新** (`FeedPurchase.vue`, `FeedUsage.vue`):
