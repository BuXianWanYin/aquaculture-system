...
public class YieldStatisticsController {
     ...
     */
    @PostMapping
    @RequiresPermission({"yield:add"}) // 需要产量新增权限
     ...
     */
    @PutMapping
    @RequiresPermission({"yield:edit"}) // 需要产量编辑权限
    ...
    @RequiresPermission({"yield:delete"}) // 需要产量删除权限
     ...
     */
    @PostMapping("/audit")
    @RequiresPermission({"yield:audit"}) // 需要产量审核权限
