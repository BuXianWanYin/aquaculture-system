...
│   │   │       │   ├── MedicinePurchaseController.java  # 药品采购控制器：处理药品采购记录的增删改查接口
│   │   │       │   ├── MedicineInventoryController.java # 药品库存控制器：处理药品库存的增删改查接口
...
│   │   │       │   ├── MedicinePurchase.java            # 药品采购实体类：对应 medicine_purchase 表，存储药品采购记录
│   │   │       │   ├── MedicineInventory.java           # 药品库存实体类：对应 medicine_inventory 表，存储药品库存信息
...
│   │   │       │   ├── MedicinePurchaseMapper.java      # 药品采购 Mapper 接口：药品采购数据访问层
│   │   │       │   ├── MedicineInventoryMapper.java    # 药品库存 Mapper 接口：药品库存数据访问层
...
│   │   │       │   ├── MedicinePurchaseService.java     # 药品采购服务接口：定义药品采购业务逻辑方法
│   │   │       │   ├── MedicineInventoryService.java    # 药品库存服务接口：定义药品库存业务逻辑方法
...
│   │   │       │       ├── MedicinePurchaseServiceImpl.java # 药品采购服务实现类：实现药品采购业务逻辑
│   │   │       │       ├── MedicineInventoryServiceImpl.java # 药品库存服务实现类：实现药品库存业务逻辑
...
- 计划录入、编辑、查询
- 计划审批流程
- 计划调整管理（支持延长周期、修改目标产量、修改投放量等调整类型）
- 计划状态管理（0-待审批，1-已通过，2-已驳回，3-执行中，4-已完成，5-已取消）
  - 状态变更规则：已通过的计划可手动修改为执行中或已完成；执行中的计划可修改为已完成
  - 已移除自动状态变更逻辑，状态变更完全由用户手动控制

### 4. 产量统计管理模块
- 产量录入与统计
- 产量审核流程（0-待审核，1-已通过，2-已驳回）
- 产量凭证管理
...
- 日常巡检记录
- 投喂记录管理
- 饲料库存、采购、使用管理（支持联动：采购自动更新库存，使用自动扣减库存）

### 6. 病害管理模块
- 病害记录管理
- 病害预防管理
- 药品采购、库存、使用管理（支持联动：采购自动更新库存，使用自动扣减库存）
- 用药记录管理（与药品库存联动）
...
1. **饲料采购 → 库存联动**
   - 新增采购：如果为新饲料，创建新库存记录；如果为已有饲料，累加库存数量
   - 更新采购：根据采购数量变化自动调整库存
   - 删除采购：如果该饲料还有其他采购记录，只减少库存；如果所有采购记录都删除，则删除对应库存记录
...
3. **库存聚合规则**
   - 库存按 `feedName` 和 `feedType` 聚合，同一名称和类型的饲料合并显示
   - 详情页显示该饲料的所有采购记录（包括批次号、保质期等）

### 药品管理联动逻辑

1. **药品采购 → 库存联动**
   - 新增采购：如果为新药品，创建新库存记录；如果为已有药品，累加库存数量
   - 更新采购：根据采购数量变化自动调整库存
   - 删除采购：如果该药品还有其他采购记录，只减少库存；如果所有采购记录都删除，则删除对应库存记录
...
3. **库存聚合规则**
   - 库存按 `medicineName` 和 `medicineType` 聚合，同一名称和类型的药品合并显示
...
- **状态变更规则**：
  - 审批通过后，计划状态自动变为"执行中"（状态3）
  - 已通过（状态1）的计划可以手动修改为"执行中"（状态3）或"已完成"（状态4）
  - 执行中（状态3）的计划可以手动修改为"已完成"（状态4）
  - 已完成（状态4）的计划不能再修改状态
  - **已移除所有自动状态变更逻辑**（如根据日期和产量自动变更状态），状态变更完全由用户手动控制
